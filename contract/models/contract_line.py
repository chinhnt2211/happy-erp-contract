                ...
                if line.date_start > line.recurring_next_date:
                    raise ValidationError(
                        self.env._(
                            ...
                            "to the start of the contract line '%(name)s'",
                            name=line.name,
                        )
                    )
            ...
            if (
                ...
                raise ValidationError(
                    self.env._(
                        ...
                        "of last invoice for the contract line '%(name)s'",
                        name=rec.name,
                    )
                )
    ...
    def _check_recurring_next_date_recurring_invoices(self):
        for rec in self:
            if not rec.recurring_next_date and (
                ...
                raise ValidationError(
                    self.env._(
                        "You must supply a date of next invoice for contract line "
                        "'%(name)s'",
                        name=rec.name,
                    )
                )
        ...
        for line in self.filtered("date_end"):
            if line.date_start and line.date_end:
                if line.date_start > line.date_end:
                    raise ValidationError(
                        self.env._(
                            "Contract line '%(name)s' start date can't be later than "
                            "end date",
                            name=line.name,
                        )
                    )
