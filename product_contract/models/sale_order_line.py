...
class SaleOrderLine(models.Model):
    ...
    def _prepare_contract_line_values(
        ...
        """
        self.ensure_one()
        # Ensure required recurrence fields are never False (contract.line requires them)
        recurring_rule_type = self.recurring_rule_type or "monthly"
        recurring_interval = self.recurring_interval if self.recurring_interval else 1
        recurring_invoicing_type = self.recurring_invoicing_type or "pre-paid"
